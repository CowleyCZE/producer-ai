...
const EditingStage: React.FC<EditingStageProps> = ({ segments, setSegments, context, onConfirm }) => {
  ...
  const handleRegenerate = async (segmentId: string) => {
    ...
    try {
      ...
      const newVariants = await regenerateSegment(segments, index, context);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 flex flex-col h-[calc(100vh-100px)]">
      <header className="flex flex-col md:flex-row justify-between items-center mb-6 bg-slate-900/50 p-4 rounded-xl border border-slate-800 gap-4">
        ...
        <button
          onClick={onConfirm}
          className="bg-green-600 hover:bg-green-500 text-white px-8 py-3 rounded-xl font-bold transition-all hover:scale-105 shadow-lg shadow-green-900/40 whitespace-nowrap uppercase tracking-wider"
        >
